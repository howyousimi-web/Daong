...
/* =========================================================
   HOME PAGE ΓÇö hero search, path buttons, scroll cue
...
/* =========================================================
   DRIVES GRID ΓÇö used on the home teaser and the full Drives page.
  ...
  container.innerHTML = drives.map((d) => {
    ...
    <article class="appeal-card" data-drive-id="${d.id}">
      ...
      <div class="card-body">
        ...
        ` : `
          <a href="drives.html" class="text-btn" style="margin-top:1rem;display:inline-block;">View &amp; pledge ΓåÆ</a>
        `}
      </div>
    </article>`;
  }).join('');
...
async function confirmPledge(driveId, drive, card, inline) {
  ...
  confirmBtn.textContent = 'SubmittingΓÇª';
  ...
  inline.innerHTML = `<div class="ai-box"><div class="ai-label">Pledge received</div>Your donation is now tracked as <strong>${donation.id}</strong>. <a href="track.html?id=${encodeURIComponent(donation.id)}" style="color:var(--gold-2);text-decoration:underline;">Track its journey ΓåÆ</a></div>`;
}

/* =========================================================
   TRACK PAGE (Citizen Lookup) ΓÇö public, no auth required
   ========================================================= */
...
async function renderTrackResult(rawId, resultEl) {
  resultEl.innerHTML = '<div class="mini-card"><p class="empty-state">SearchingΓÇª</p></div>';
  ...
  const tl = logic.STAGES.map((stage, i) => {
    ...
    return `<div class="tl-item ${cls}">
      <div class="tl-dot">${cp ? (isFlaggedStage ? '!' : 'Γ£ô') : (i + 1)}</div>
      <div class="tl-body">
        ...
        ${isFlaggedStage ? '<div class="tl-note" style="color:var(--red);">Flagged for verification ΓÇö under review.</div>' : ''}
      </div></div>`;
  }).join('');
    ...
    <div class="mini-card">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:10px;margin-bottom:6px;flex-wrap:wrap;">
        <div>
          ...
          <div class="donor" style="font-size:17px;">${logic.money(donation.amountPhp)} ΓÇö ${escapeHtml(donation.category)}</div>
...
async function runSummarize(donation, btn) {
  ...
  btn.disabled = true; btn.textContent = 'SummarizingΓÇª';
  slot.innerHTML = '<div class="ai-box"><div class="ai-label"><span class="ai-pulse"></span>AI Summary</div>Reading the checkpoint historyΓÇª</div>';
  const { ok, text } = await window.donationLogic.summarizeJourney(donation);
  slot.innerHTML = `<div class="ai-box${ok ? '' : ' is-error'}"><div class="ai-label">${ok ? '<span class="ai-pulse"></span>AI Summary' : 'AI Summary ΓÇö Unavailable'}</div>${escapeHtml(text)}</div>`;
  btn.disabled = false; btn.textContent = 'Ask AI to summarize this journey';
}

/* =========================================================
   DASHBOARD ΓÇö donor stat chip (existing) + Admin Ops (new,
...
async function handleRegisterDonation() {
  ...
  if (!result.ok) { showFormError('Could not register that donation ΓÇö please try again.'); return; }
  ...
  wrap.innerHTML = donations.slice().reverse().map((s) => `
    <div class="shipment">
      <div class="left">
        ...
        <ul class="cp-list">${s.checkpoints.map((c) => `<li><span class="cp-loc">${c.stage} ΓÇö ${escapeHtml(c.loc)}</span><span>${c.time}</span></li>`).join('')}</ul>
      </div>
      <div class="right">
...
async function runExplain(id, btn) {
  ...
  btn.disabled = true; btn.textContent = 'Asking AIΓÇª';
  slot.innerHTML = '<div class="ai-box"><div class="ai-label"><span class="ai-pulse"></span>AI Insight</div>Reading checkpoint dataΓÇª</div>';
  const { ok, text } = await window.donationLogic.explainFlag(donation);
  slot.innerHTML = `<div class="ai-box${ok ? '' : ' is-error'}"><div class="ai-label">${ok ? '<span class="ai-pulse"></span>AI Insight' : 'AI Insight ΓÇö Unavailable'}</div>${escapeHtml(text)}</div>`;
