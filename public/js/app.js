...
const DAONG_RECAPTCHA_SITE_KEY = '6Le10KMTAAAAAPbnCUTU2dC1wrF4qt3Hpb8wJcSv';
...
function ensureRecaptchaLoaded() {
  return new Promise((resolve) => {
    ...
    script.src = `https://www.google.com/recaptcha/enterprise.js?render=${DAONG_RECAPTCHA_SITE_KEY}`;
    script.async = true;
    script.defer = true;
...
async function getRecaptchaToken(action = 'LOGIN') {
  ...
  try {
    return await window.grecaptcha.enterprise.execute(DAONG_RECAPTCHA_SITE_KEY, { action });
  } catch (_err) {
    return null;
  }
}
...
/* =========================================================
   0. THEME TOGGLE ΓÇö light / dark, persisted across reloads
...
/* =========================================================
   0c. STICKY HEADER SHADOW ΓÇö a light shadow once the page has
...
/* =========================================================
   0b. SCROLL-REVEAL ΓÇö fades static sections in as they enter
...
class NotificationEngine {
  ...
  render() {
    ...
    if (!this.items.length) {
      this.list.innerHTML = '<li class="notif-empty">You\'re all caught up ΓÇö no notifications yet.</li>';
    } else {
...
class AuthStateManager {
  ...
  // role: 'donor' (default) or 'admin' ΓÇö this is a labeled demo login,
  ...
  renderBanner() {
    ...
    if (this.state === 'expired') {
      ...
      this.banner.textContent = 'Session expired. Re-authenticatingΓÇª';
